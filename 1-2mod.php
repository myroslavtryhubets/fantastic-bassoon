...
<div class="card-header">
        <h4 class="my-0 font-weight-normal">mod2</h4>
    </div>
<div class="card-body">
        <div>
  ...
  <div>
    <label class="desc" for="a">XOR = </label>
       <span id="XOR"></span>
  </div>  
</div>
</div>
  ...
  function mod2(){
    var A = $("#A").val().split(' ').join('');
    var B = $("#B").val().split(' ').join('');
    var out = "";
    if(A.length > B.length){
      bmod = B.length;
      for (var i = 0; i < A.length; i++) {
        if(parseInt(A[i]) + parseInt(B[i % bmod]) == 0){
          out+="0";
        }
        else if(parseInt(A[i]) + parseInt(B[i % bmod]) == 1){
          out+="1";
        }
        else if(parseInt(A[i]) + parseInt(B[i % bmod]) == 2){
          out+="0";
        }
      }
    }
      ...
      for (var i = 0; i < B.length; i++) {
        if(parseInt(B[i]) + parseInt(A[i % bmod]) == 0){
          out+="0";
        }
        else if(parseInt(B[i]) + parseInt(A[i % bmod]) == 1){
          out+="1";
        }
        else if(parseInt(B[i]) + parseInt(A[i % bmod]) == 2){
          out+="0";
        }
      }
    }
      ...
      for (var i = 0; i < B.length; i++) {
        if(parseInt(B[i]) + parseInt(A[i]) == 0){
          out+="0";
        }
        else if(parseInt(B[i]) + parseInt(A[i]) == 1){
          out+="1";
        }
        else if(parseInt(B[i]) + parseInt(A[i]) == 2){
          out+="0";
        }
      }
    }
  ...
  $(document).ready(function() {mod2();});
  $("textarea").keyup(function(event) {mod2();});
